...
5. Only the project foundation and design system exist (see DEVELOPER.md
   status markers). Do not document or reference systems that do not exist.
   The `/design` route is an internal preview surface, not a product page.

## Conventions

6. JavaScript only. Do not introduce TypeScript.
7. File naming: components `PascalCase.jsx`; hooks `use-thing.js`;
   everything else `kebab-case.js`. Tests: `*.test.js(x)` next to the code
   or in `src/test/`.
8. Components: shadcn primitives in `src/components/ui/` (edit sparingly,
   keep shadcn structure); Lokker-specific components in
...
10. Errors: throw `AppError` subclasses (src/lib/errors.js). Never
